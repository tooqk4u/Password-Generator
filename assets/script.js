...
// 3. validate - only isNumbers and length of password,
// 4. prompt- window.confirm()  to ask user if they want uppercase, lowercase,numbers,special characters
// 5. validate - a choice was made from prompt
// 6. Need to randomize confirmed choices with a loop
// Assignment Code
...
let generatePassword = function () {
  ...
  if (passwordLength < 8 || passwordLength > 128 || isNaN(passwordLength)) {
    ...
    if (
      ...
      alert(
        "You must chose at least one character for your password. Please try again"
      );
    }
    ...
    for (let i = 0; i < passwordLength; i++) {
      if (isUpperCase == true && finalPassword.length < passwordLength) {
        let randomPickUpperCase = Math.floor(Math.random() * upperCase.length);
        let pickAnUpperCaseLetter = upperCase.charAt(randomPickUpperCase);
        finalPassword = finalPassword + pickAnUpperCaseLetter;
